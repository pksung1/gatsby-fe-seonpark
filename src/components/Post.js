...
const Post = ({title, description, date, tags, className}) => {
  const wrapper = classNames(
    ...
    border-red-800 rounded-xl p-4 flex flex-row justify-between dark:shadow-gray-600 overflow-hidden", className)

  return (
    <div className={wrapper}>
      <div className='dark:text-white flex-1'>
        <h3 className='text-2xl text-ellipsis'>{title}</h3>
